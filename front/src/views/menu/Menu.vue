...
export default {
  ...
  methods: {
    listMenus(){
      this.axios.get("/api/admin/menus",{params: {keywords: this.condition.keywords}})
          .then(({data})=>{
            if(data.flag === false) {
      ...
      if (menu) {
        ...
        switch (type) {
          case 1:
            ...
            this.mode = 0;
            this.menuForm.parentName = JSON.parse(JSON.stringify(menu.name));
            break;
          case 2:
    ...
    changeDisable(data) {
      ...
      this.condition.whether = data.isHidden;
      this.axios.post("/api/admin/changeMenu",this.condition).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: "修改成功,请重新登录以加载左侧菜单"
          });
          this.condition.id = null;
          this.condition.whether = null;
        } else {
          this.$notify.error({
            title: "失败",
    ...
    saveOrUpdateMenu(){
      ...
      switch (this.mode) {
        case 0:
          ...
          break;
        case 1:
          this.axios.post("/api/admin/menus/update", this.menuForm).then(({ data }) => {
            if (data.flag) {
              this.$notify.success({
                title: "成功",
                message: "操作成功"
